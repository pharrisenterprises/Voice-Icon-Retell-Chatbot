...
export default function EmbedPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hostParam = (u.searchParams.get('host') || u.searchParams.get('hostcontrol') || '').toLowerCase();
      wantsAuto = ['1', 'true', 'yes', 'auto', 'force'].includes(autostartParam);
      wantsManual = ['0', 'false', 'no', 'off', 'manual'].includes(autostartParam);
      hostControlled = ['1', 'true', 'yes', 'host', 'embed'].includes(hostParam);
    } catch {}
    setHostControl(hostControlled);

    let inIframe = false;
    try { inIframe = window.self !== window.top; } catch { inIframe = true; }
    const shouldAutoBoot = !hostControlled && (wantsAuto || (!wantsManual && !inIframe));
    setNeedsUserStart(!shouldAutoBoot && !hostControlled);

    if (!shouldAutoBoot) {
      wantListeningRef.current = false;
      setMicOn(false);
      setStatus(hostControlled ? 'Tap Chat to Speak' : 'Turn Mic On to Speak');
    } else {
      setSoundInstant(true);
    }
...
<speak version="1.0" xml:lang="en-US">
  ...
  // -------------------- Host-controlled resume/pause --------------------
  function resumeConversation(immediateSpeech = false) {
    if (!userUnlockedRef.current) {
      handleUserStart(immediateSpeech);
      return;
    }
    try { window.speechSynthesis?.cancel(); } catch {}
    setSoundInstant(true);
  ...
  useEffect(() => {
    window.widgetStop = () => pauseConversation();
  ...
  useEffect(() => {
    function onMessage(evt) {
      try {
        ...
        if (data.type === 'avatar-widget:close' || data.type === 'avatar-widget:pause') {
          pauseConversation();
        } else if (data.type === 'avatar-widget:open') {
          const immediate = data?.immediate !== false;
          if (!userUnlockedRef.current) handleUserStart(immediate);
          else resumeConversation(immediate);
  ...
  return (
    <div className="wrap">
      ...
      {needsUserStart && !userUnlocked && !hostControl && (
